...
public class TaskContentProvider extends ContentProvider {
    ...
    // Thực hiện insert để xử lý các yêu cầu chèn một hàng dữ liệu mới
    @Override
    public Uri insert(@NonNull Uri uri, ContentValues values) {
        // Hoàn thành (1) Truy cập task database (để ghi dữ liệu mới)
        final SQLiteDatabase db = mTaskDbHelper.getWritableDatabase();

        // Hoàn thành (2) Viết code URI matching để match thư mục task
        int match = sUriMatcher.match(uri);
        Uri returnUri; // URI trả về

        switch (match) {
            case TASKS:
                // Hoàn thành (3) Insert giá trị mới vào database
                ...
                break;
            // Hoàn thành (4) Đặt giá trị cho returnUri và viết trường hợp mặc định cho URI không xác định
            // Trường hợp mặc định ném ra một UnsupportedOperationException
            default:
                throw new UnsupportedOperationException("Unknown uri: " + uri);
        }

        // Hoàn thành (5) Thông báo cho resolver nếu uri đã được thay đổi và trả lại URI vừa được insert
        getContext().getContentResolver().notifyChange(uri, null);

        // Trả về uri đã xây dựng (trỏ vào hàng dữ liệu mới chèn vào)
        return returnUri;
    }
