    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        new ItemTouchHelper(new ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.LEFT | ItemTouchHelper.RIGHT) {
            ...
            public void onSwiped(RecyclerView.ViewHolder viewHolder, int swipeDir) {
                ...
                // Hoàn thành (1) Xây dựng URI cho item sẽ xóa
                // [Gợi ý] Dùng getTag (từ code adapter) để lấy id của item được vuốt
                // Retrieve the id of the task to delete
                int id = (int) viewHolder.itemView.getTag();

                // Xây dựng uri thích hợp với id hàng String được nối
                String stringId = Integer.toString(id);
                Uri uri = TaskContract.TaskEntry.CONTENT_URI;
                uri = uri.buildUpon().appendPath(stringId).build();

                // Hoàn thành (2) Xóa hàng dữ liệu đơn bằng ContentResolver
                getContentResolver().delete(uri, null, null);

                // Hoàn thành (3) Khởi động lại loader để tái truy vấn tất cả các task sau khi xóa
    ...
    public Loader<Cursor> onCreateLoader(int id, final Bundle loaderArgs) {
        ...
        return new AsyncTaskLoader<Cursor>(this) {
            ...
            public Cursor loadInBackground() {
                ...
                // Truy vấn và tải tất cả dữ liệu task trong background; sắp xếp theo mức ưu tiên
