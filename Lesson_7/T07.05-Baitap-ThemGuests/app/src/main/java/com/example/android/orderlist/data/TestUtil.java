...
public class TestUtil {
    ...
    public static void insertFakeData(SQLiteDatabase db){
        ...
        cv.put(OrderlistContract.OrderlistEntry.COLUMN_GUEST_NAME, "John");
        cv.put(OrderlistContract.OrderlistEntry.COLUMN_PARTY_SIZE, 12);
        list.add(cv);

        cv = new ContentValues();
        cv.put(OrderlistContract.OrderlistEntry.COLUMN_GUEST_NAME, "Tim");
        cv.put(OrderlistContract.OrderlistEntry.COLUMN_PARTY_SIZE, 2);
        list.add(cv);

        cv = new ContentValues();
        cv.put(OrderlistContract.OrderlistEntry.COLUMN_GUEST_NAME, "Jessica");
        cv.put(OrderlistContract.OrderlistEntry.COLUMN_PARTY_SIZE, 99);
        list.add(cv);

        cv = new ContentValues();
        cv.put(OrderlistContract.OrderlistEntry.COLUMN_GUEST_NAME, "Larry");
        cv.put(OrderlistContract.OrderlistEntry.COLUMN_PARTY_SIZE, 1);
        list.add(cv);

        cv = new ContentValues();
        cv.put(OrderlistContract.OrderlistEntry.COLUMN_GUEST_NAME, "Kim");
